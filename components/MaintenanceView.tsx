...
const MaintenanceView: React.FC = () => {
    ...
    const getTechnicianAvailabilityBadgeClass = (status: TechnicianStatus) => {
        switch (status) {
            case TechnicianStatus.Available: return 'bg-green-500/20 text-green-400';
            case TechnicianStatus.OnSite: return 'bg-yellow-500/20 text-yellow-400';
            case TechnicianStatus.OffDuty: return 'bg-slate-500/20 text-slate-400';
            default: return 'bg-slate-500/20 text-slate-400';
        }
    };
    ...
    const techniciansMap = new Map(TECHNICIANS.map(tech => [tech.name, tech]));
                    ...
                    <tbody>
                        {filteredTasks.length > 0 ? (
                            filteredTasks.map((task: MaintenanceTask) => {
                                const technician = techniciansMap.get(task.assignedTechnician);
                                return (
                                    <tr key={task.id} className="bg-slate-800 border-b border-slate-700 hover:bg-slate-700/50">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                {technician && (
                                                    <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getTechnicianAvailabilityBadgeClass(technician.availability)}`}>
