...
const TABS: { id: AlertDetailTab; label: string }[] = [
    { id: 'details', label: 'Details' },
    { id: 'timeline', label: 'Timeline' },
    { id: 'kanban', label: 'Kanban' },
];

const AlertDetailView: React.FC<{ alert: Alert | undefined; onBack: () => void; }> = ({ alert, onBack }) => {
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        <AlertDetailHeader alert={currentAlert} onBack={onBack} />
            ...
            <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                {TABS.map((tab) => (
                    <button
                        ...
                        className={`${ activeTab === tab.id ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-slate-400 hover:text-slate-200 hover:border-slate-300'} whitespace-nowrap capitalize py-3 px-1 border-b-2 font-medium text-sm transition-colors`}
                    >
                        {tab.label}
                    </button>
                ))}
