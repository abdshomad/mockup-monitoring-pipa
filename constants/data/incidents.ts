...
export const INCIDENTS: Incident[] = [
    {
        ...
        log: [
            { timestamp: getRelativeTimestamp({ minutes: -10 }), entry: 'Incident declared from critical alert A-98721.', operator: 'System' },
            { timestamp: getRelativeTimestamp({ minutes: -9 }), entry: 'Andi assigned as Incident Commander.', operator: 'System Admin' },
            { timestamp: getRelativeTimestamp({ minutes: -5 }), entry: 'Confirmed patrol dispatch (Alpha-3). Confirmed ETA is 15 minutes from incident start.', operator: 'Andi (Control Room)' },
            { timestamp: getRelativeTimestamp({ minutes: -2 }), entry: 'Notified local law enforcement of potential criminal activity.', operator: 'Andi (Control Room)', notes: 'Reference number LE-45921.' },
        ],
        followUpTasks: [
    ...
    },
    {
        ...
        log: [
            { timestamp: getRelativeTimestamp({ days: -2, hours: -4 }), entry: 'Incident declared due to multiple communication timeout alerts.', operator: 'Operator 1' },
            { timestamp: getRelativeTimestamp({ days: -2, hours: -3, minutes: -30 }), entry: 'Network team engaged to investigate gateway BL-GW-02.', operator: 'Operator 1' },
            { timestamp: getRelativeTimestamp({ days: -2, hours: -1 }), entry: 'Network team confirmed power surge. Initiating remote reboot.', operator: 'Tech Team' },
            { timestamp: getRelativeTimestamp({ days: -2, hours: 0, minutes: 45 }), entry: 'Gateway BL-GW-02 back online. All linked sensors are reporting normally.', operator: 'Tech Team' },
            { timestamp: getRelativeTimestamp({ days: -2, hours: 1 }), entry: 'Incident resolved. Root cause: power surge. Follow-up task created to install improved surge protection.', operator: 'Operator 1' },
        ],
        followUpTasks: [
